''' Interior Fork Module '''

import math
import atum.geometry as geo
from . import TransmutationNode

class InteriorFork(TransmutationNode):
    ...
    def __init__(self, parent, polygon):
        ...
        self.outer_circle = geo.Circle(self.polygon.center, self.polygon.radius)
        self.inner_circle = geo.Circle(self.polygon.center, self.interior_radius)
    ...
    def get_forking_polygons(self):
        ...
        return [gen_polygon(i, point)
                for i, point in enumerate(self._forking_polygon().points)]

    def get_interior_polygon(self):
        interior_polygon = self.polygon.copy()
        interior_polygon.radius = self.interior_radius
    ...
    def _forking_polygon(self):
        forking_polygon = self.polygon.copy()
        forking_polygon.radius = self.fork_radius
        return forking_polygon
